...
import { Briefcase, Plus, X, Trash2, Edit2, ChevronDown, ChevronRight } from "lucide-react";
...
export default function TiposServicio() {
  ...
  const fetchConfigsForTipo = async (tipoId: number, nombreTipo: string) => {
    try {
      ...
      const filtered = allConfigs.filter((c: ConfigCosto) => c.tipo_servicio === nombreTipo);
  ...
  const toggleExpand = (tipo: TipoServicio) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingTipo
        ? `/api/tipos-servicio/${editingTipo.id}`
        : "/api/tipos-servicio";
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-600">
              Gestión de tipos de servicio para configuración de costos
            </p>
          </div>
          <button
        ...
        <div className="space-y-4">
          {tipos.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-200">
              ...
              <p className="text-gray-600 mb-4">No hay tipos de servicio registrados</p>
              <button
                onClick={openNewModal}
                    ...
                    <div className="border-t border-gray-200 bg-gray-50 p-4">
                      ...
                      {isLoadingConfigs ? (
                        ...
                        <p className="text-sm text-gray-500 text-center py-4">
                          No hay configuraciones usando este tipo de servicio
                        </p>
                      ) : (
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <span className="text-sm text-gray-900">
                                  {config.tipo_transporte || "-"} / {config.tipo_embalaje || "-"}
                                </span>
                              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: Recogida, Embalaje, Transporte"
                />
              </div>
