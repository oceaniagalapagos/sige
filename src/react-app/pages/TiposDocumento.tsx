...
import { FileText, Plus, X, Trash2, Edit2, ChevronDown, ChevronRight } from "lucide-react";
...
export default function TiposDocumento() {
  ...
  const fetchDocsForTipo = async (tipoId: number, nombreTipo: string) => {
    try {
      setLoadingDocs(tipoId);
      const response = await fetch("/api/documentos");
      const allDocs = await response.json();
      const filtered = allDocs.filter((d: Documento) => d.tipo_documento === nombreTipo);
      setDocsMap(prev => ({ ...prev, [tipoId]: filtered }));
    } catch (error) {
  ...
  const toggleExpand = (tipo: TipoDocumento) => {
    if (expandedId === tipo.id) {
      setExpandedId(null);
    } else {
      setExpandedId(tipo.id);
      if (!docsMap[tipo.id]) {
        fetchDocsForTipo(tipo.id, tipo.nombre_tipo);
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingTipo
        ? `/api/tipos-documento/${editingTipo.id}`
        : "/api/tipos-documento";
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/tipos-documento/${id}`, {
        method: "DELETE",
      });
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-600">
              Gestión de tipos de documento para embarques
            </p>
          </div>
          <button
        ...
        <div className="space-y-4">
          {tipos.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-200">
              ...
              <p className="text-gray-600 mb-4">No hay tipos de documento registrados</p>
              <button
                onClick={openNewModal}
            ...
            tipos.map((tipo) => {
              ...
              const docs = docsMap[tipo.id] || [];
              const isLoadingDocs = loadingDocs === tipo.id;
                    ...
                    <div className="border-t border-gray-200 bg-gray-50 p-4">
                      ...
                      {isLoadingDocs ? (
                        ...
                        <p className="text-sm text-gray-500 text-center py-4">
                          No hay documentos usando este tipo
                        </p>
                      ) : (
                        <div className="space-y-2">
                          ...
                          {docs.length > 5 && (
                            <p className="text-sm text-gray-500 text-center py-2">
                              Y {docs.length - 5} documentos más...
                            </p>
                          )}
                        </div>
                      )}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: Factura, Guía de Remisión, Certificado"
                />
              </div>
