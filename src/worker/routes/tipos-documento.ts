...
export const tiposDocumentoRoutes = new Hono<{ Bindings: Env }>();
...
tiposDocumentoRoutes.get("/", authMiddleware, async (c) => {
  const { results } = await c.env.DB.prepare(
    "SELECT * FROM tipos_documento ORDER BY nombre_tipo ASC"
  ).all();
...
tiposDocumentoRoutes.get("/:id", authMiddleware, async (c) => {
  const id = c.req.param("id");

  const tipo = await c.env.DB.prepare(
    "SELECT * FROM tipos_documento WHERE id = ?"
  )
    .bind(id)
    .first();

  if (!tipo) {
    return c.json({ error: "Tipo de documento no encontrado" }, 404);
  }
...
tiposDocumentoRoutes.patch(
  ...
  async (c) => {
    ...
    await c.env.DB.prepare(
      `UPDATE tipos_documento SET ${updates.join(", ")} WHERE id = ?`
    )
      .bind(...params)
      .run();
...
tiposDocumentoRoutes.delete("/:id", authMiddleware, adminOnly, async (c) => {
  const id = c.req.param("id");

  // Verificar si hay documentos usando este tipo
  const docsUsando = await c.env.DB.prepare(
    "SELECT COUNT(*) as count FROM documentos WHERE tipo_documento = (SELECT nombre_tipo FROM tipos_documento WHERE id = ?)"
  )
    .bind(id)
    .first<{ count: number }>();

  if (docsUsando && docsUsando.count > 0) {
    return c.json(
      { error: "No se puede eliminar: hay documentos usando este tipo" },
      400
    );
  }

  await c.env.DB.prepare("DELETE FROM tipos_documento WHERE id = ?")
    .bind(id)
    .run();
