...
export const tiposServicioRoutes = new Hono<{ Bindings: Env }>();
...
tiposServicioRoutes.get("/", authMiddleware, async (c) => {
  const { results } = await c.env.DB.prepare(
    "SELECT * FROM tipos_servicio ORDER BY nombre_tipo ASC"
  ).all();
...
tiposServicioRoutes.get("/:id", authMiddleware, async (c) => {
  const id = c.req.param("id");

  const tipo = await c.env.DB.prepare(
    "SELECT * FROM tipos_servicio WHERE id = ?"
  )
    .bind(id)
    .first();

  if (!tipo) {
    return c.json({ error: "Tipo de servicio no encontrado" }, 404);
  }
...
tiposServicioRoutes.post(
  ...
  async (c) => {
    ...
    const result = await c.env.DB.prepare(
      "INSERT INTO tipos_servicio (nombre_tipo, descripcion) VALUES (?, ?)"
    )
...
tiposServicioRoutes.patch(
  ...
  async (c) => {
    ...
    await c.env.DB.prepare(
      `UPDATE tipos_servicio SET ${updates.join(", ")} WHERE id = ?`
    )
      .bind(...params)
      .run();
...
tiposServicioRoutes.delete("/:id", authMiddleware, adminOnly, async (c) => {
  const id = c.req.param("id");

  // Verificar si hay configuraciones usando este tipo
  const configsUsando = await c.env.DB.prepare(
    "SELECT COUNT(*) as count FROM configuracion_costos WHERE tipo_servicio = (SELECT nombre_tipo FROM tipos_servicio WHERE id = ?)"
  )
    .bind(id)
    .first<{ count: number }>();
  ...
  if (configsUsando && configsUsando.count > 0) {
    return c.json(
      { error: "No se puede eliminar: hay configuraciones de costos usando este tipo de servicio" },
      400
    );
  }

  await c.env.DB.prepare("DELETE FROM tipos_servicio WHERE id = ?")
    .bind(id)
    .run();
